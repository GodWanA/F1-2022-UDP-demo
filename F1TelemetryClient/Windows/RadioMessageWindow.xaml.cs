...
namespace F1TelemetryApp.Windows
{
    ...
    public partial class RadioMessageWindow : Window, IConnectUDP, IDisposable
    {
                ...
                for (int i = 0; i < packet.CarDamageData.Length; i++)
                {
                    ...
                    if (ok)
                    {
                        ...
                        if (currDemage != null && lastDemage != null)
                        {
                            ...
                            if (msg.Length > 0 && driver != null) this.AppendMessage(driver.ParticipantName, msg.ToString(), u.PickTeamColor(driver.TeamID).Color);
                        }
                    }
                }
            }
        ...
        private void AppendMessage(string driverName, string message, Color teamColor)
        {
        ...
        internal void AppendMessage(string message, Color teamColor)
        {
            this.stackPanel_messages.Children.Add(
                    new RadioMessageCard(
                        message,
                        teamColor,
